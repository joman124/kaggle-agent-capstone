...
- **guardrails.py** — shared first-pass guardrail checks.
- **agents/writer.py** — Writer agent (promoted from the old `step1_writer.py`
  single-file prototype); uses `GEMINI_WRITER_MODEL` (pro-tier by default).
...
- `.env` with `GEMINI_API_KEY`, `GEMINI_MODEL=gemini-2.5-flash` (other agents),
  and optionally `GEMINI_WRITER_MODEL` (defaults to `gemini-pro-latest`)
