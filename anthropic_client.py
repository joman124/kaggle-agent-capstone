...
def generate(model: str, prompt: str, system_instruction: str = None,
    ...
    for _ in range(_MAX_RESUMES + 1):
        try:
            ...
            # Do NOT truncate the provider's message. The Gemini-era lesson
            # (commit "Show the exact quota Google names on a 429") was that
            # the useful part -- which limit tripped, and its value -- is the
            # tail of the error, and a [:300] cut threw away exactly that.
            # Anthropic also puts the limit in response headers, so surface
            # those too rather than making John guess which cap he hit.
            hdrs = getattr(getattr(e, "response", None), "headers", None) or {}
            limits = [f"  {k}: {v}" for k, v in hdrs.items()
            ...
            raise SystemExit(
                ...
                + ("\nThe exact limit Anthropic reported:\n" + "\n".join(limits) + "\n"
                   if limits else "")
                + f"\nAnthropic's full message:\n  {e}\n"
            )
